...
class R_DPMADDPG(Trainer):
    def __init__(self, args, num_agents, policies, policy_mapping_fn, device=None, episode_length=None, actor_update_interval=1):
        """
        R_DPMADDPG, modified from R_MADDPG

        Trainer class for recurrent MADDPG. See parent class for more information.
        ...
        ##### dp communication #####
        # data center is used to store message at t-1, send to actor at t
        self.data_center = dict()
        self.integration_center = dict()
        ##### end #####

    def get_update_info(self, update_policy_id, obs_batch, act_batch, avail_act_batch, msg_batch):
        """
        ...
        for p_id in self.policy_ids:
            ...
            batched_msg_seq = np.concatenate(msg_batch[p_id], axis=1)
            ...
            with torch.no_grad():
                ...
                # use obs and msg
                pol_nact_seq, _, _ = policy.get_actions(batched_obs_seq, batched_msg_seq,
    ...
    def train_policy_on_batch(self, update_policy_id, batch):
        """See parent class."""
        return self.comm_shared_train_policy_on_batch_v1(update_policy_id, batch)

    def comm_shared_train_policy_on_batch(self, update_policy_id, batch):
        return self.comm_shared_train_policy_on_batch_v1(update_policy_id, batch)

    def comm_shared_train_policy_on_batch_v1(self, update_policy_id, batch):
        ...
        avail_act_batch, \
        importance_weights, idxes, message_batch = batch
        train_info = {}
        ...
        pol_agents_obs_seq = np.float32(pol_agents_obs_seq)
        pol_agents_msg_seq = np.concatenate(message_batch[update_policy_id], axis=1)[:-1]
        ...
        cent_act_sequence_buffer, act_sequences, act_sequence_replace_ind_start, cent_nact_sequence = \
            self.get_update_info(update_policy_id, obs_batch, act_batch, avail_act_batch, message_batch)
        # Critic update:
        ...
        if update_actor:
            ...
            # send msg using other agents' sender
            receiver_id = int(update_policy_id.split('_')[-1])
            msg_sent_list = []
            for other_policy_key in self.policies.keys():
                if other_policy_key==update_policy_id:
                    continue
                other_policy = self.policies[other_policy_key]
                tmp_obs = np.concatenate(obs_batch[other_policy_key], axis=1)[:-1]
                tmp_act = np.concatenate(act_batch[other_policy_key], axis=1)
                if self.args.msg_include_id:
                    ones_id = np.ones( [tmp_obs.shape[0], tmp_obs.shape[1],  1], dtype=np.float32)
                    input_info = np.concatenate([tmp_obs, tmp_act, ones_id], axis=-1)
                else:
                    input_info = np.concatenate([tmp_obs, tmp_act], axis=-1)
                input_info = torch.tensor(input_info)
                msg = other_policy.sender(input_info, True)
                if self.args.msg_include_id:
                    receiver_ones = torch.ones([msg.shape[0],msg.shape[1],1] , dtype=torch.float32).to(msg.device)
                    msg = torch.cat([msg, receiver_ones], dim=-1)
                msg_sent_list.append(msg)
            # receive
            pol_agents_msg_seq = torch.cat(msg_sent_list, dim=-1)
            pol_agents_msg_seq = update_policy.receiver(pol_agents_msg_seq)
            ##### end #####    

            # get all the actions from actor, with gumbel softmax to differentiate through the samples
            policy_act_seq, _, _ = update_policy.get_actions(pol_agents_obs_seq, pol_agents_msg_seq,
            ...
            # zero grad all optimizers we want
            update_policy.critic_optimizer.zero_grad()
            update_policy.actor_optimizer.zero_grad()
            update_policy.receiver_optimizer.zero_grad()
            for other_policy_key in self.policies.keys():
                if other_policy_key==update_policy_id:
                    continue
                other_policy = self.policies[other_policy_key]
                other_policy.sender_optimizer.zero_grad()
            # backward
            actor_loss.backward()
            # grad clip
            actor_grad_norm = torch.nn.utils.clip_grad_norm_(update_policy.actor.parameters(),
                                                             self.args.max_grad_norm)
            if self.args.protocol_grad_clip:
                receiver_grad_norm = torch.nn.utils.clip_grad_norm_(update_policy.receiver.parameters(),
                                                                self.args.max_grad_norm)
                sender_grad_norms = 0.
                for other_policy_key in self.policies.keys():
                    if other_policy_key==update_policy_id:
                        continue
                    other_policy = self.policies[other_policy_key]
                    sender_grad_norm = torch.nn.utils.clip_grad_norm_(other_policy.sender.parameters(),
                                                                self.args.max_grad_norm)
                    sender_grad_norms += sender_grad_norm.clone()
            # step all optimizer
            update_policy.actor_optimizer.step()
            update_policy.receiver_optimizer.step()
            for other_policy_key in self.policies.keys():
                if other_policy_key==update_policy_id:
                    continue
                other_policy = self.policies[other_policy_key]
                other_policy.sender_optimizer.step()
            ...
            if self.args.protocol_grad_clip:
                train_info['receiver_grad_norm'] = receiver_grad_norm
                train_info['sender_grad_norm_summation'] = sender_grad_norms
    ...
    def message_integration(self):
        ...
        for p_i in self.policy_ids:
            message_i = []
            agent_i = int(p_i.split('_')[-1])
            policy_i = self.policies[p_i]
            for p_j in self.policy_ids:
                agent_j = int(p_j.split('_')[-1])
                if agent_i == agent_j:
                    continue
                # collect
                message_ji = self.data_center[(agent_j, agent_i)].to(device)
                message_i.append(message_ji)
            message_i = [msg.unsqueeze(0) for msg in message_i]
            message_i = torch.cat(message_i)
            message_i = policy_i.receiver(message_i) # 1x128x5
            self.integration_center[agent_i] = message_i
            messages_received.append(message_i)
        return messages_received


    def dpmac_communicate(self, observations, actions):
        """
        Make one step's communication
        """
        self.data_center = dict() # refresh
        for p_i, observation_i, action_i in zip(self.policy_ids, observations, actions):
            agent_i = int(p_i.split('_')[-1])
            policy_i = self.policies[p_i]
            ...
            for p_j in self.policy_ids:
                ...
                if agent_j == agent_i:
                    continue
                input = np.concatenate( [ observation_i, action_i ] , axis=-1)
                input = to_torch(input).float()
                message_ij = policy_i.sender(input, True)
                self.data_center[(agent_i, agent_j)] = message_ij
        self.message_integration()
        

    def dpmac_init_communicate(self, batch_size):
        """
        ...
        for p_i in self.policy_ids:
            ...
            for p_j in self.policy_ids:
                ...
                if(agent_i==agent_j):
                    continue
                if self.args.msg_include_id:
                    nothing = torch.zeros( [batch_size, policy_i.sender.input_shape -1] ) # minus 1 for concat of agent id
                    input = torch.cat([nothing, torch.tensor([agent_j])]).to(policy_i.sender.device) # send to j
                else:
                    input = torch.zeros([batch_size, policy_i.sender.input_shape]).to(policy_i.sender.device)
                message_ij = policy_i.sender(input) # mean
                self.data_center[(agent_i, agent_j)] = message_ij
