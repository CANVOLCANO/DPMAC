...
from offpolicy.utils.rec_buffer import RecReplayBuffer, PrioritizedRecReplayBuffer, CommRecReplayBuffer
...
class RecRunner(object):
    ...
    def __init__(self, config):
        ...
        self.use_communication = self.args.use_communication # whether to use communication
        ...
        self.achieve_dp = self.args.achieve_dp
        ...
        if self.algorithm_name == "rdpmaddpg":
            assert self.actor_train_interval_step == 1, (
                "rmaddpg only supports actor_train_interval_step=1.")
            from offpolicy.algorithms.r_dpmaddpg.algorithm.rDPMADDPGPolicy import R_DPMADDPGPolicy as Policy
            from offpolicy.algorithms.r_dpmaddpg.r_dpmaddpg import R_DPMADDPG as TrainAlgo
        else:
        ...
        if self.use_communication:
            self.train = self.batch_train_comm
        else:
        ...
        if self.use_communication:
            # the replay buffer that stores the msg information
            self.buffer = CommRecReplayBuffer(self.policy_info,
                                        self.policy_agents,
                                        self.buffer_size,
                                        self.episode_length,
    ...
    def batch_train_comm(self):
        """Do a gradient update for all policies, with communication"""
        self.trainer.prep_training()
        # gradient updates
        self.train_infos = []
        update_actor = False
        for p_id in self.policy_ids:
            if self.use_per:
                ...
                sample = self.buffer.sample(self.batch_size)
            update_method = self.trainer.comm_shared_train_policy_on_batch if self.use_same_share_obs else self.trainer.comm_cent_train_policy_on_batch
            # all sample -> generate msg -> msg receive -> actor-critic -> optimize
            train_info, new_priorities, idxes = update_method(p_id, sample)
            update_actor = train_info['update_actor']
            if self.use_per:
                self.buffer.update_priorities(idxes, new_priorities, p_id)
