...
def get_dim_from_space(space):
    if isinstance(space, Box):
        ...
        if isinstance(space[0], Discrete) and isinstance(space[1], Discrete) and isinstance(space[2], gym.spaces.multi_binary.MultiBinary): # customized for TJ
            dim = get_dim_from_space(space[2]) + 2
        else:
